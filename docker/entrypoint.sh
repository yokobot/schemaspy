...
exec java -jar /usr/local/lib/schemaspy/schemaspy*.jar -dp $DRIVER_PATH -o $SCHEMASPY_OUTPUT -t mysql -host ${DB_HOST} -u ${DB_USER} -db ${DB_NAME} -s ${DB_NAME} -port 3306 -p ${DB_PASS}
